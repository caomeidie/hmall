<?php if (!defined('THINK_PATH')) exit();?><form id="pagerForm" method="post" action="<?php echo U('goods_brand/index');?>">
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="<?php echo U('goods_brand/index');?>" method="post">
	<div class="searchBar">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			...
			<li><a class="add" href="<?php echo U('goods_brand/add');?>" target="navTab"><span>添加</span></a></li>
			<li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="check" postType="string" href="<?php echo U('goods_brand/del');?>" class="delete"><span>删除</span></a></li>
	...
	<table class="table" width="100%" layoutH="138">
		...
		<tbody>
		<?php if(is_array($brand_list)): foreach($brand_list as $key=>$value): ?><tr target="sid" rel="<?php echo ($value['brand_id']); ?>">
    		    <td><label><input type="checkbox" name="check" value="<?php echo ($value['brand_id']); ?>" /></label></td>
        		<td><?php echo ($value['brand_name']); ?></td>
                <td><?php if($value['brand_type'] != 0): ?><img src="/hmall/Public/upload/brand/<?php echo ($value['brand_pic']); ?>" height="20px"><?php endif; ?></td>
                <td><?php if($value['brand_type'] == 0): ?>文字<?php else: ?>图片<?php endif; ?></td>
                <td><?php echo ($value['brand_sort']); ?></td>
                <td>
                    <a title="删除" target="ajaxTodo" href="<?php echo U('goods_brand/del',array('uid'=>$value['brand_id']));?>" class="btnDel">删除</a>
					<a title="编辑" target="navTab" href="<?php echo U('goods_brand/edit',array('uid'=>$value['brand_id']));?>" class="btnEdit">编辑</a>
                </td>
